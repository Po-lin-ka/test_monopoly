create or replace package body monopoly as
   ...
   procedure pay_rent(p_participant_id number, p_cell_id number, p_dice number);
   procedure apply_chance(p_participant_id number, p_dice number);
   procedure start_auction(p_game_id number, p_cell_id number);
   procedure close_auction(p_auction_id number, p_force boolean default false);
   ...
   function participant_game(v_participant_id number) return number is
      v_game_id number;
   begin
   ...
   begin
      ...
      if v_game_status = 'АКТИВНА'
         and v_turn_state in ('ОЖИДАНИЕ_БРОСКА', 'ОЖИДАНИЕ_ПОКУПКИ', 'ОЖИДАНИЕ_УЛУЧШЕНИЯ', 'ЗАВЕРШЕНИЕ_ХОДА',
                              'ПОКРЫТИЕ_ДОЛГА')
   ...
   begin
      ...
      insert into "ЖУРНАЛ_ДЕЙСТВИЙ" (
         ...
         (p_game_id, p_participant_id, p_cell_id, p_action_code, p_amount, p_event_text, sysdate);
   end;

   procedure return_properties_to_bank(p_participant_id number) is
   begin
      update "ВЛАДЕНИЯ"
         set "ID_ВЛАДЕЛЬЦА" = null, "КОЛВО_ДОМОВ" = 0, "ЗАЛОЖЕНА" = 0
       where "ID_ВЛАДЕЛЬЦА" = p_participant_id;
   end;

   procedure enter_debt_or_bankruptcy(p_participant_id number) is
      v_game_id number;
      v_balance number;
      available number;
   begin
      v_game_id := participant_game(p_participant_id);
      select "БАЛАНС" into v_balance
        from "УЧАСТНИКИ"
       where "ID_УЧАСТНИКА" = p_participant_id;
      if v_balance >= 0 then
         update "ИГРЫ"
            set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
   ...
   begin
      ...
      select rawtohex(standard_hash(p_password, 'SHA256')) into password_hash
        from dual;
      insert into "ПОЛЬЗОВАТЕЛИ" ("ЛОГИН", "ПАРОЛЬ_ХЭШ", "ДАТА_РЕГИСТРАЦИИ") values
         (trim(p_login), password_hash, sysdate);
   exception
   ...
   begin
      select rawtohex(standard_hash(p_password, 'SHA256')) into password_hash
        from dual;
      select "ID_ПОЛЬЗОВАТЕЛЯ" into id
        from "ПОЛЬЗОВАТЕЛИ"
   ...
   procedure create_game(
      p_user_id number, p_game_name varchar2, p_max_players number default 4,
      p_room_password varchar2 default null, p_game_id out number
   ) is
      v_count number;
      start_id number;
   ...
   begin
      ...
      on games."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_ПОЛЬЗОВАТЕЛЯ" = p_user_id
         and games."КОД_СТАТУСА_ИГРЫ" in ('ОЖИДАНИЕ', 'ПРОВЕРКА_ГОТОВНОСТИ', 'АКТИВНА')
         and u."КОД_СТАТУСА_УЧАСТНИКА" in ('В_ЛОББИ', 'АКТИВЕН');
      if v_count > 0 then
         raise_application_error(-20023, 'Пользователь уже в незавершённой игре');
      end if;
      select "ID_КЛЕТКИ" into start_id
        from "КЛЕТКИ"
       where "ПОЗИЦИЯ" = 1;
      if p_room_password is null then
         room_password_hash := null;
      else
         select rawtohex(standard_hash(p_room_password, 'SHA256')) into room_password_hash
           from dual;
      end if;
      insert into "ИГРЫ" (
         "ID_ХОСТА", "КОД_СТАТУСА_ИГРЫ", "НАЗВАНИЕ", "ПАРОЛЬ_ХЭШ", "ДАТА_СОЗДАНИЯ", "МАКС_ИГРОКОВ"
      ) values
         (p_user_id, 'ОЖИДАНИЕ', trim(p_game_name), room_password_hash, sysdate, p_max_players)
      returning "ID_ИГРЫ" into p_game_id;
      insert into "УЧАСТНИКИ" (
         "ID_ИГРЫ", "ID_ПОЛЬЗОВАТЕЛЯ", "ID_ПОЗИЦИИ", "БАЛАНС", "КОД_СТАТУСА_УЧАСТНИКА", "ГОТОВ", "КОЛ_ТАЙМАУТОВ"
      ) values
         (p_game_id, p_user_id, start_id, c_start_fund, 'В_ЛОББИ', 0, 0);
   end;
   ...
   begin
      open v_result for
         select games."ID_ИГРЫ", games."НАЗВАНИЕ", games."МАКС_ИГРОКОВ", count(
               case when u."КОД_СТАТУСА_УЧАСТНИКА" = 'В_ЛОББИ' then 1 end
         ) "ЗАНЯТО",
            case when games."ПАРОЛЬ_ХЭШ" is null then 0 else 1 end "ЕСТЬ_ПАРОЛЬ"
         from "ИГРЫ" games
         left join "УЧАСТНИКИ" u
            on u."ID_ИГРЫ" = games."ID_ИГРЫ"
         where games."КОД_СТАТУСА_ИГРЫ" = 'ОЖИДАНИЕ'
         group by games."ID_ИГРЫ", games."НАЗВАНИЕ", games."МАКС_ИГРОКОВ", games."ПАРОЛЬ_ХЭШ",
            games."ДАТА_СОЗДАНИЯ"
         having count(case when u."КОД_СТАТУСА_УЧАСТНИКА" = 'В_ЛОББИ' then 1 end) < games."МАКС_ИГРОКОВ"
         order by games."ДАТА_СОЗДАНИЯ" desc;
      return v_result;
   end;

   procedure join_game(p_user_id number, p_game_id number, p_room_password varchar2 default null) is
   ...
   begin
      ...
      if p_room_password is null then
         ...
         select rawtohex(standard_hash(p_room_password, 'SHA256')) into entered_password_hash
           from dual;
      end if;
      select "КОД_СТАТУСА_ИГРЫ", "ПАРОЛЬ_ХЭШ", "МАКС_ИГРОКОВ"
        into v_status, v_room_password_hash, v_max_players
        from "ИГРЫ"
       where "ID_ИГРЫ" = p_game_id;
      if v_status <> 'ОЖИДАНИЕ' then
         raise_application_error(-20030, 'Комната недоступна');
      end if;
      if v_room_password_hash is not null
      ...
      select "ID_КЛЕТКИ" into v_start_cell_id
        from "КЛЕТКИ"
       where "ПОЗИЦИЯ" = 1;
      begin
         select "ID_УЧАСТНИКА", "КОД_СТАТУСА_УЧАСТНИКА"
           into v_participant_id, v_player_status
           from "УЧАСТНИКИ"
          where "ID_ИГРЫ" = p_game_id
         ...
         update "УЧАСТНИКИ"
            set "КОД_СТАТУСА_УЧАСТНИКА" = 'В_ЛОББИ', "ГОТОВ" = 0, "ОЧЕРЕДЬ_ХОДА" = null, "КОЛ_ТАЙМАУТОВ" = 0,
                "БАЛАНС" = c_start_fund, "ID_ПОЗИЦИИ" = v_start_cell_id
          where "ID_УЧАСТНИКА" = v_participant_id;
      exception
         when no_data_found then
            insert into "УЧАСТНИКИ" ("ID_ИГРЫ", "ID_ПОЛЬЗОВАТЕЛЯ", "ID_ПОЗИЦИИ", "БАЛАНС", "КОД_СТАТУСА_УЧАСТНИКА") values
               (p_game_id, p_user_id, v_start_cell_id, c_start_fund, 'В_ЛОББИ');
      end;
   end;

   procedure abandon_waiting_game(p_user_id number, p_game_id number) is
   ...
   begin
      ...
      or v_status not in ('ОЖИДАНИЕ', 'ПРОВЕРКА_ГОТОВНОСТИ') then
         raise_application_error(-20040, 'Нельзя забросить игру');
      end if;
      update "ИГРЫ"
         set "КОД_СТАТУСА_ИГРЫ" = 'ЗАБРОШЕНА', "ДАТА_ЗАВЕРШЕНИЯ" = sysdate, "ID_ТЕКУЩЕГО_УЧАСТНИКА" = null,
             "КОД_СОСТОЯНИЯ_ХОДА" = null
       where "ID_ИГРЫ" = p_game_id;
      update "УЧАСТНИКИ"
         set "КОД_СТАТУСА_УЧАСТНИКА" = 'ПОКИНУЛ', "ГОТОВ" = 0
       where "ID_ИГРЫ" = p_game_id
         and "КОД_СТАТУСА_УЧАСТНИКА" = 'В_ЛОББИ';
      add_action(p_game_id, null, null, 'ИГРА_ЗАБРОШЕНА', null);
   end;

   procedure leave_lobby(p_participant_id number) is
      v_game_id number;
      v_user_id number;
      v_host_user_id number;
      v_status varchar2(40);
   begin
      ...
      if v_user_id = v_host_user_id then
         abandon_waiting_game(v_user_id, v_game_id);
         return;
      end if;
      if v_status not in ('ОЖИДАНИЕ', 'ПРОВЕРКА_ГОТОВНОСТИ') then
   ...
   procedure set_ready(p_participant_id number, p_ready number) is
      v_game_id number;
      v_status varchar2(40);
      v_count number;
      ready number;
   begin
      ...
      if p_ready = 1 then
         add_action(v_game_id, p_participant_id, null, 'ПОДТВЕРЖДЕНИЕ_ГОТОВНОСТИ', null);
      end if;
      select count(*), nvl(sum("ГОТОВ"), 0)
        into v_count, ready
        from "УЧАСТНИКИ"
       where "ID_ИГРЫ" = v_game_id
         and "КОД_СТАТУСА_УЧАСТНИКА" = 'В_ЛОББИ';
      ...
      then
         update "ИГРЫ"
            set "КОД_СТАТУСА_ИГРЫ" = 'ПРОВЕРКА_ГОТОВНОСТИ', "ВРЕМЯ_НАЧАЛА_ХОДА" = sysdate
          where "ID_ИГРЫ" = v_game_id;
      else
         update "ИГРЫ"
            set "КОД_СТАТУСА_ИГРЫ" = 'ОЖИДАНИЕ', "ВРЕМЯ_НАЧАЛА_ХОДА" = null
          where "ID_ИГРЫ" = v_game_id;
      end if;
   end;
   ...
   begin
      ...
      for player in (
         ...
         update "УЧАСТНИКИ"
            set "ОЧЕРЕДЬ_ХОДА" = turn_number, "КОД_СТАТУСА_УЧАСТНИКА" = 'АКТИВЕН',
                "БАЛАНС" = floor(c_start_fund / v_count), "КОЛ_ТАЙМАУТОВ" = 0, "ГОТОВ" = 1,
                "ПОСЛЕДНЯЯ_СВЯЗЬ" = sysdate
          where "ID_УЧАСТНИКА" = player."ID_УЧАСТНИКА";
      end loop;
      insert into "ВЛАДЕНИЯ" ("ID_ИГРЫ", "ID_КЛЕТКИ", "ID_ВЛАДЕЛЬЦА", "КОЛВО_ДОМОВ", "ЗАЛОЖЕНА")
         select p_game_id, "ID_КЛЕТКИ", null, 0, 0
           from "КЛЕТКИ"
          where "ТИП" in ('Улица', 'Коммунальная');
      select "ID_УЧАСТНИКА" into first_id
        from "УЧАСТНИКИ"
       where "ID_ИГРЫ" = p_game_id
         and "ОЧЕРЕДЬ_ХОДА" = 1;
      update "ИГРЫ"
         set "КОД_СТАТУСА_ИГРЫ" = 'АКТИВНА', "ДАТА_СТАРТА" = sysdate, "ID_ТЕКУЩЕГО_УЧАСТНИКА" = first_id,
             "ВРЕМЯ_НАЧАЛА_ХОДА" = sysdate, "КОД_СОСТОЯНИЯ_ХОДА" = 'ОЖИДАНИЕ_БРОСКА'
       where "ID_ИГРЫ" = p_game_id;
   end;

   function get_game_state(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
   begin
      open v_result for
         select games."ID_ИГРЫ", games."НАЗВАНИЕ", games."КОД_СТАТУСА_ИГРЫ",
            sg."НАИМЕНОВАНИЕ" "СТАТУС_ИГРЫ", games."КОД_СОСТОЯНИЯ_ХОДА",
            sh."НАИМЕНОВАНИЕ" "СОСТОЯНИЕ_ХОДА", games."ID_ТЕКУЩЕГО_УЧАСТНИКА", games."ID_ПОБЕДИТЕЛЯ",
            games."ВРЕМЯ_НАЧАЛА_ХОДА", games."ДАТА_СТАРТА", games."ДАТА_ЗАВЕРШЕНИЯ", games."ID_ХОСТА",
            games."МАКС_ИГРОКОВ", a."ID_АУКЦИОНА", ca."НАЗВАНИЕ" "АУКЦИОН_КЛЕТКА", a."СТАРТ_ЦЕНА",
            ...
            case
               ...
               greatest(0, ceil(c_ready_seconds -(sysdate - games."ВРЕМЯ_НАЧАЛА_ХОДА") * 86400))
            end "СЕКУНД_ДО_СТАРТА",
            case
               when games."КОД_СТАТУСА_ИГРЫ" = 'АКТИВНА'
               and games."ВРЕМЯ_НАЧАЛА_ХОДА" is not null then
               greatest(0, ceil(c_turn_minutes * 60 -(sysdate - games."ВРЕМЯ_НАЧАЛА_ХОДА") * 86400))
            end "СЕКУНД_ХОДА", (
   ...
   function get_game_participants(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
      v_game_id number;
   begin
      v_game_id := participant_game(p_participant_id);
      open v_result for
         select u."ID_УЧАСТНИКА", u."ID_ПОЛЬЗОВАТЕЛЯ", p."ЛОГИН", u."БАЛАНС", c."ПОЗИЦИЯ",
            c."НАЗВАНИЕ" "КЛЕТКА", u."ОЧЕРЕДЬ_ХОДА", u."КОД_СТАТУСА_УЧАСТНИКА",
            s."НАИМЕНОВАНИЕ" "СТАТУС", u."ГОТОВ", u."КОЛ_ТАЙМАУТОВ"
         from "УЧАСТНИКИ" u
         join "ПОЛЬЗОВАТЕЛИ" p
         ...
         order by nvl(u."ОЧЕРЕДЬ_ХОДА", 99), u."ID_УЧАСТНИКА";
      return v_result;
   end;

   function get_player_properties(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
   begin
      open v_result for
         select v."ID_ВЛАДЕНИЯ", c."ID_КЛЕТКИ", c."НАЗВАНИЕ", c."ТИП", v."КОЛВО_ДОМОВ",
            v."ЗАЛОЖЕНА", c."ЦЕНА_ПОКУПКИ", floor(c."ЦЕНА_ПОКУПКИ" / 2) "ЗАЛОГОВАЯ_СТОИМОСТЬ",
            floor(ceil(c."ЦЕНА_ПОКУПКИ" * 0.25) / 2) "СТОИМОСТЬ_ПРОДАЖИ_УРОВНЯ",
            ceil(c."ЦЕНА_ПОКУПКИ" *(1 + c_mortgage_interest)) "СТОИМОСТЬ_ВЫКУПА",
            case when v."ЗАЛОЖЕНА" = 0 and v."КОЛВО_ДОМОВ" = 0 then 1 else 0 end "МОЖНО_ЗАЛОЖИТЬ"
         from "ВЛАДЕНИЯ" v
         join "КЛЕТКИ" c
   ...
   function get_active_auction(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
      v_game_id number;
   begin
      v_game_id := participant_game(p_participant_id);
      open v_result for
         select a."ID_АУКЦИОНА", a."ID_КЛЕТКИ", c."НАЗВАНИЕ", a."СТАРТ_ЦЕНА", a."ДАТА_НАЧАЛА",
            s."ID_УЧАСТНИКА", p."ЛОГИН", s."СУММА", s."ДАТА_ВРЕМЯ"
         from "АУКЦИОНЫ" a
         join "КЛЕТКИ" c
         ...
         order by s."СУММА" desc nulls last, s."ДАТА_ВРЕМЯ";
      return v_result;
   end;

   procedure roll_and_move(p_participant_id number, p_dice out number) is
      v_game_id number;
      v_turn_state varchar2(40);
      v_balance number;
      v_position number;
   ...
   begin
      ...
      select x."КОД_СОСТОЯНИЯ_ХОДА", u."БАЛАНС", c."ПОЗИЦИЯ"
        into v_turn_state, v_balance, v_position
        from "УЧАСТНИКИ" u
        join "ИГРЫ" x
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
        join "КЛЕТКИ" c
      ...
      p_dice := trunc(dbms_random.value(1, 7));
      v_new_position := mod(v_position - 1 + p_dice, 12) + 1;
      if v_position - 1 + p_dice >= 12 then
         bonus := c_start_bonus;
         update "УЧАСТНИКИ"
            set "БАЛАНС" = "БАЛАНС" + bonus
          where "ID_УЧАСТНИКА" = p_participant_id;
      end if;
      select "ID_КЛЕТКИ" into v_cell_id
        from "КЛЕТКИ"
       where "ПОЗИЦИЯ" = v_new_position;
      update "УЧАСТНИКИ"
         set "ID_ПОЗИЦИИ" = v_cell_id
       where "ID_УЧАСТНИКА" = p_participant_id;
      add_action(v_game_id, p_participant_id, null, 'БРОСОК_КУБИКА', p_dice);
      add_action(v_game_id, p_participant_id, v_cell_id, 'ПОСЕЩЕНИЕ_КЛЕТКИ', null);
      if bonus > 0 then
         add_action(v_game_id, p_participant_id, v_cell_id, 'БОНУС_СТАРТА', bonus);
      end if;
      process_cell(p_participant_id, p_dice);
   end;

   procedure process_cell(p_participant_id number, p_dice number) is
      v_game_id number;
      v_cell_id number;
      ...
      v_level number;
   begin
      select u."ID_ИГРЫ", u."ID_ПОЗИЦИИ", c."ТИП"
        into v_game_id, v_cell_id, v_cell_type
        from "УЧАСТНИКИ" u
        join "КЛЕТКИ" c
      on c."ID_КЛЕТКИ" = u."ID_ПОЗИЦИИ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      if v_cell_type = 'Старт' then
         ...
         apply_chance(p_participant_id, p_dice);
      else
         select "ID_ВЛАДЕЛЬЦА", "ЗАЛОЖЕНА", "КОЛВО_ДОМОВ"
           into v_owner_id, v_mortgaged, v_level
           from "ВЛАДЕНИЯ"
          where "ID_ИГРЫ" = v_game_id
            and "ID_КЛЕТКИ" = v_cell_id;
         if v_owner_id is null then
            update "ИГРЫ"
               set "КОД_СОСТОЯНИЯ_ХОДА" = 'ОЖИДАНИЕ_ПОКУПКИ'
             where "ID_ИГРЫ" = v_game_id;
            ...
            if v_mortgaged = 1 then
               ...
               pay_rent(p_participant_id, v_cell_id, p_dice);
            end if;
         else
            if v_cell_type = 'Улица'
               and v_mortgaged = 0
               and v_level < 3
            then
               update "ИГРЫ"
                  set "КОД_СОСТОЯНИЯ_ХОДА" = 'ОЖИДАНИЕ_УЛУЧШЕНИЯ'
                where "ID_ИГРЫ" = v_game_id;
            else
               update "ИГРЫ"
                  set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
   ...
   procedure advance_to_next_player(p_game_id number) is
      v_current_player_id number;
      v_turn_order number;
      v_next_player_id number;
   begin
      select "ID_ТЕКУЩЕГО_УЧАСТНИКА" into v_current_player_id
        from "ИГРЫ"
       where "ID_ИГРЫ" = p_game_id;
      select "ОЧЕРЕДЬ_ХОДА" into v_turn_order
        from "УЧАСТНИКИ"
       where "ID_УЧАСТНИКА" = v_current_player_id;
      begin
         select "ID_УЧАСТНИКА" into v_next_player_id
           from (
            select "ID_УЧАСТНИКА"
              from "УЧАСТНИКИ"
             where "ID_ИГРЫ" = p_game_id
      ...
      update "ИГРЫ"
         set "ID_ТЕКУЩЕГО_УЧАСТНИКА" = v_next_player_id, "ВРЕМЯ_НАЧАЛА_ХОДА" = sysdate,
             "КОД_СОСТОЯНИЯ_ХОДА" = 'ОЖИДАНИЕ_БРОСКА'
       where "ID_ИГРЫ" = p_game_id;
   end;

   procedure end_turn(p_game_id number) is
      v_turn_state varchar2(40);
      v_current_player_id number;
      v_balance number;
      v_status varchar2(40);
   begin
      ...
      select "КОД_СОСТОЯНИЯ_ХОДА", "ID_ТЕКУЩЕГО_УЧАСТНИКА"
        into v_turn_state, v_current_player_id
        from "ИГРЫ"
       where "ID_ИГРЫ" = p_game_id;
   ...
   procedure buy_property(p_participant_id number) is
   ...
   begin
      v_game_id := require_current_player(p_participant_id);
      select u."ID_ПОЗИЦИИ", x."КОД_СОСТОЯНИЯ_ХОДА", u."БАЛАНС"
        into current_cell_id, v_turn_state, v_balance
        from "УЧАСТНИКИ" u
        join "ИГРЫ" x
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      ...
      select v."ID_ВЛАДЕЛЬЦА", c."ЦЕНА_ПОКУПКИ", c."ТИП"
        into v_owner_id, v_price, v_cell_type
        from "ВЛАДЕНИЯ" v
        join "КЛЕТКИ" c
      on c."ID_КЛЕТКИ" = v."ID_КЛЕТКИ"
      ...
      or v_cell_type not in ('Улица', 'Коммунальная')
      or v_balance < v_price then
         raise_application_error(-20071, 'Клетку нельзя купить');
      end if;
      update "УЧАСТНИКИ"
         set "БАЛАНС" = "БАЛАНС" - v_price
       where "ID_УЧАСТНИКА" = p_participant_id;
      update "ВЛАДЕНИЯ"
         set "ID_ВЛАДЕЛЬЦА" = p_participant_id
       where "ID_ИГРЫ" = v_game_id
         and "ID_КЛЕТКИ" = current_cell_id;
      add_action(v_game_id, p_participant_id, current_cell_id, 'ПОКУПКА_СОБСТВЕННОСТИ', v_price);
      update "ИГРЫ"
         set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
       where "ID_ИГРЫ" = v_game_id;
   end;

   procedure decline_purchase(p_participant_id number) is
      current_cell_id number;
      v_game_id number;
      v_turn_state varchar2(40);
      v_owner_id number;
   begin
      ...
      select u."ID_ПОЗИЦИИ", x."КОД_СОСТОЯНИЯ_ХОДА"
        into current_cell_id, v_turn_state
        from "УЧАСТНИКИ" u
        join "ИГРЫ" x
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      ...
      select "ID_ВЛАДЕЛЬЦА" into v_owner_id
        from "ВЛАДЕНИЯ"
       where "ID_ИГРЫ" = v_game_id
         and "ID_КЛЕТКИ" = current_cell_id;
      ...
      add_action(v_game_id, p_participant_id, current_cell_id, 'ОТКАЗ_ОТ_ПОКУПКИ', null);
      start_auction(v_game_id, current_cell_id);
   end;
   ...
   begin
      ...
      if v_cell_type = 'Коммунальная' then
         ...
         return p_dice *
            case when v_count >= 2 then 50 else 25 end;
      end if;
      ...
      return nvl(rent, 0) *
         case when v_group_count > 0 and v_count = v_group_count then 2 else 1 end;
   end;

   procedure pay_rent(p_participant_id number, p_cell_id number, p_dice number) is
      v_game_id number;
   ...
   begin
      ...
      add_action(v_game_id, p_participant_id, p_cell_id, 'ОПЛАТА_АРЕНДЫ', rent, recipient_login);
      enter_debt_or_bankruptcy(p_participant_id);
   end;

   procedure apply_chance(p_participant_id number, p_dice number) is
      v_game_id number;
      target number;
   ...
   begin
      v_game_id := participant_game(p_participant_id);
      select "ID_ПОЗИЦИИ" into chance_cell
        from "УЧАСТНИКИ"
       where "ID_УЧАСТНИКА" = p_participant_id;
      select "ID_ЦЕЛЕВОЙ_КЛЕТКИ", "ТИП_ЭФФЕКТА", "СУММА_ИЗМЕНЕНИЯ", "ТЕКСТ_СОБЫТИЯ"
        into target, v_cell_type, v_card_amount, v_card_text
        from (
         select "ID_ЦЕЛЕВОЙ_КЛЕТКИ", "ТИП_ЭФФЕКТА", "СУММА_ИЗМЕНЕНИЯ", "ТЕКСТ_СОБЫТИЯ"
           from "КАРТЫ_ШАНСА"
      ...
      add_action(v_game_id, p_participant_id, chance_cell, 'КАРТА_ШАНСА', v_card_amount, v_card_text);
      if v_cell_type = 'Премия' then
         update "УЧАСТНИКИ"
            set "БАЛАНС" = "БАЛАНС" + v_card_amount
          where "ID_УЧАСТНИКА" = p_participant_id;
         update "ИГРЫ"
            set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
          where "ID_ИГРЫ" = v_game_id;
         ...
         select c."ПОЗИЦИЯ" into v_old_position
           from "УЧАСТНИКИ" u
           join "КЛЕТКИ" c
         on c."ID_КЛЕТКИ" = u."ID_ПОЗИЦИИ"
          where u."ID_УЧАСТНИКА" = p_participant_id;
         select "ПОЗИЦИЯ" into v_new_position
           from "КЛЕТКИ"
         ...
         if v_new_position <= v_old_position then
            ...
            update "УЧАСТНИКИ"
               set "БАЛАНС" = "БАЛАНС" + bonus
             where "ID_УЧАСТНИКА" = p_participant_id;
            add_action(v_game_id, p_participant_id, target, 'БОНУС_СТАРТА', bonus);
         end if;
         update "УЧАСТНИКИ"
            set "ID_ПОЗИЦИИ" = target
          where "ID_УЧАСТНИКА" = p_participant_id;
         add_action(v_game_id, p_participant_id, target, 'ПОСЕЩЕНИЕ_КЛЕТКИ', null);
         process_cell(p_participant_id, p_dice);
      end if;
   end;

   procedure build_house(p_participant_id number) is
      current_cell_id number;
      v_game_id number;
      v_turn_state varchar2(40);
      v_cell_type varchar2(30);
   ...
   begin
      ...
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      select c."ТИП", ceil(c."ЦЕНА_ПОКУПКИ" * 0.25), v."ID_ВЛАДЕЛЬЦА", v."ЗАЛОЖЕНА", v."КОЛВО_ДОМОВ"
        into v_cell_type, v_price, v_owner_id, v_mortgaged, v_level
        from "ВЛАДЕНИЯ" v
        join "КЛЕТКИ" c
      on c."ID_КЛЕТКИ" = v."ID_КЛЕТКИ"
       where v."ID_ИГРЫ" = v_game_id
      ...
      update "УЧАСТНИКИ"
       ...
       where "ID_УЧАСТНИКА" = p_participant_id;
      update "ВЛАДЕНИЯ"
         set "КОЛВО_ДОМОВ" = "КОЛВО_ДОМОВ" + 1
       where "ID_ИГРЫ" = v_game_id
         and "ID_КЛЕТКИ" = current_cell_id;
      add_action(v_game_id, p_participant_id, current_cell_id, 'ПОКУПКА_УЛУЧШЕНИЯ', v_price);
      update "ИГРЫ"
         set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
       where "ID_ИГРЫ" = v_game_id;
   end;

   procedure decline_improvement(p_participant_id number) is
      current_cell_id number;
      v_game_id number;
      v_turn_state varchar2(40);
      v_owner_id number;
   begin
      ...
      add_action(v_game_id, p_participant_id, current_cell_id, 'ОТКАЗ_ОТ_УЛУЧШЕНИЯ', null);
      update "ИГРЫ"
         set "КОД_СОСТОЯНИЯ_ХОДА" = 'ЗАВЕРШЕНИЕ_ХОДА'
       where "ID_ИГРЫ" = v_game_id;
   end;

   procedure sell_buildings(p_participant_id number, p_ownership_id number) is
   begin
      process_properties(p_participant_id, number_list(), number_list(p_ownership_id), true);
   end;

   procedure resolve_debt(p_participant_id number, p_mortgage_ids number_list, p_sale_ids number_list) is
   begin
   ...
   begin
      ...
      if v_turn_state is null
      or (v_turn_state <> 'ПОКРЫТИЕ_ДОЛГА' and not (p_sale_before_roll and v_turn_state = 'ОЖИДАНИЕ_БРОСКА')) then
      ...
      if v_mortgage_count > 0 then
         select count(*), nvl(sum(floor(c."ЦЕНА_ПОКУПКИ" / 2)), 0)
      ...
      if v_sale_count > 0 then
         select count(*), v_total + nvl(sum(floor(ceil(c."ЦЕНА_ПОКУПКИ" * 0.25) / 2)), 0)
           into v_count, v_total
           from "ВЛАДЕНИЯ" v
           join "КЛЕТКИ" c
         on c."ID_КЛЕТКИ" = v."ID_КЛЕТКИ"
      ...
      if v_mortgage_count > 0 then
         for r in (
            select v."ID_ВЛАДЕНИЯ" v_ownership_id, v."ID_КЛЕТКИ" v_cell_id, floor(c."ЦЕНА_ПОКУПКИ" / 2) v_amount
              from "ВЛАДЕНИЯ" v
              join "КЛЕТКИ" c
            on c."ID_КЛЕТКИ" = v."ID_КЛЕТКИ"
             where v."ID_ВЛАДЕНИЯ" in (
            ...
            add_action(v_game_id, p_participant_id, r.v_cell_id, 'ЗАЛОГ_СОБСТВЕННОСТИ', r.v_amount);
         end loop;
      end if;
      if v_sale_count > 0 then
         for r in (
            select v."ID_ВЛАДЕНИЯ" v_ownership_id, v."ID_КЛЕТКИ" v_cell_id,
            ...
            update "ВЛАДЕНИЯ"
               set "КОЛВО_ДОМОВ" = "КОЛВО_ДОМОВ" - 1
             where "ID_ВЛАДЕНИЯ" = r.v_ownership_id;
            add_action(v_game_id, p_participant_id, r.v_cell_id, 'ПРОДАЖА_ПОСТРОЕК', r.v_amount);
         end loop;
      end if;
      update "УЧАСТНИКИ"
         set "БАЛАНС" = "БАЛАНС" + v_total
       where "ID_УЧАСТНИКА" = p_participant_id;
   ...
   procedure redeem_property(p_participant_id number, p_ownership_id number) is
      v_game_id number;
      v_turn_state varchar2(40);
      v_owner_id number;
      v_mortgaged number;
   ...
   begin
      v_game_id := require_current_player(p_participant_id);
      select x."КОД_СОСТОЯНИЯ_ХОДА", u."БАЛАНС"
        into v_turn_state, v_balance
        from "УЧАСТНИКИ" u
        join "ИГРЫ" x
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      ...
      select v."ID_ВЛАДЕЛЬЦА", v."ЗАЛОЖЕНА", c."ЦЕНА_ПОКУПКИ", v."ID_КЛЕТКИ"
        into v_owner_id, v_mortgaged, v_price, v_cell_id
        from "ВЛАДЕНИЯ" v
        join "КЛЕТКИ" c
      on c."ID_КЛЕТКИ" = v."ID_КЛЕТКИ"
       where v."ID_ВЛАДЕНИЯ" = p_ownership_id
      ...
      update "УЧАСТНИКИ"
         set "БАЛАНС" = "БАЛАНС" - v_amount
       where "ID_УЧАСТНИКА" = p_participant_id;
      update "ВЛАДЕНИЯ"
         set "ЗАЛОЖЕНА" = 0
       where "ID_ВЛАДЕНИЯ" = p_ownership_id;
      add_action(v_game_id, p_participant_id, v_cell_id, 'СНЯТИЕ_ЗАЛОГА', v_amount);
   end;
   ...
   begin
      ...
      update "ИГРЫ"
         set "КОД_СОСТОЯНИЯ_ХОДА" = 'ПРОВЕДЕНИЕ_АУКЦИОНА'
       where "ID_ИГРЫ" = p_game_id;
   end;

   procedure make_bid(p_auction_id number, p_participant_id number, p_amount number) is
      v_game_id number;
      v_status varchar2(40);
   ...
   begin
      ...
      lock_game(v_game_id);
      select "ID_ИГРЫ", "КОД_СТАТУСА_АУКЦИОНА", "СТАРТ_ЦЕНА", "ДАТА_НАЧАЛА"
        into v_game_id, v_status, v_start_price, started
        from "АУКЦИОНЫ"
       where "ID_АУКЦИОНА" = p_auction_id;
      select u."ID_ИГРЫ", u."КОД_СТАТУСА_УЧАСТНИКА", u."БАЛАНС", x."ID_ТЕКУЩЕГО_УЧАСТНИКА"
        into v_player_game_id, v_player_status, v_balance, v_initiator_id
        from "УЧАСТНИКИ" u
        join "ИГРЫ" x
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      ...
      if p_amount <> 0
         and (p_amount < v_start_price or p_amount > v_balance)
      then
   ...
   procedure close_auction(p_auction_id number, p_force boolean default false) is
      v_game_id number;
      v_cell_id number;
      v_status varchar2(40);
      started date;
      ...
      answered number;
   begin
      select a."ID_ИГРЫ", a."ID_КЛЕТКИ", a."КОД_СТАТУСА_АУКЦИОНА", a."ДАТА_НАЧАЛА", x."ID_ТЕКУЩЕГО_УЧАСТНИКА"
        into v_game_id, v_cell_id, v_status, started, v_initiator_id
        from "АУКЦИОНЫ" a
        join "ИГРЫ" x
      ...
      if not p_force and sysdate < started + c_auction_seconds / 86400
         and not (eligible > 0 and answered = eligible)
      then
         return;
      end if;
      begin
         select "ID_УЧАСТНИКА", "СУММА"
           into winner, v_amount
           from (
            select s."ID_УЧАСТНИКА", s."СУММА"
              from "СТАВКИ" s
              join "УЧАСТНИКИ" u
            on u."ID_УЧАСТНИКА" = s."ID_УЧАСТНИКА"
             where s."ID_АУКЦИОНА" = p_auction_id
             ...
             order by s."СУММА" desc, s."ДАТА_ВРЕМЯ", s."ID_СТАВКИ"
           )
          where rownum = 1;
      exception
         when no_data_found then
      ...
      if winner is not null then
         ...
         update "ВЛАДЕНИЯ"
            set "ID_ВЛАДЕЛЬЦА" = winner
          where "ID_ИГРЫ" = v_game_id
            and "ID_КЛЕТКИ" = v_cell_id;
         update "АУКЦИОНЫ"
            set "ID_ПОБЕДИТЕЛЯ" = winner, "ФИНАЛ_ЦЕНА" = v_amount, "КОД_СТАТУСА_АУКЦИОНА" = 'ЗАВЕРШЕН',
                "ДАТА_ОКОНЧАНИЯ" = sysdate
          where "ID_АУКЦИОНА" = p_auction_id;
         add_action(v_game_id, winner, v_cell_id, 'АУКЦИОН', v_amount);
      else
         update "АУКЦИОНЫ"
            set "КОД_СТАТУСА_АУКЦИОНА" = 'НЕ_СОСТОЯЛСЯ', "ДАТА_ОКОНЧАНИЯ" = sysdate
          where "ID_АУКЦИОНА" = p_auction_id;
         add_action(v_game_id, null, v_cell_id, 'АУКЦИОН', null);
      end if;
   ...
   procedure handle_timeout(p_game_id number) is
      v_participant_id number;
      v_timeout_count number;
      v_balance number;
   begin
      select "ID_ТЕКУЩЕГО_УЧАСТНИКА" into v_participant_id
        from "ИГРЫ"
       where "ID_ИГРЫ" = p_game_id;
      select "КОЛ_ТАЙМАУТОВ" into v_timeout_count
        from "УЧАСТНИКИ"
       where "ID_УЧАСТНИКА" = v_participant_id;
      if v_timeout_count >= 1 then
         ...
         update "УЧАСТНИКИ"
          ...
          where "ID_УЧАСТНИКА" = v_participant_id;
         add_action(p_game_id, v_participant_id, null, 'ТАЙМ_АУТ', c_timeout_penalty);
         select "БАЛАНС" into v_balance
           from "УЧАСТНИКИ"
          where "ID_УЧАСТНИКА" = v_participant_id;
         if v_balance < 0 then
   ...
   procedure check_game_timer(p_game_id number) is
      v_turn_state varchar2(40);
      started date;
      v_game_status varchar2(40);
      v_auction_id number;
   ...
   begin
      ...
      if v_turn_state in ('ОЖИДАНИЕ_БРОСКА', 'ОЖИДАНИЕ_ПОКУПКИ', 'ОЖИДАНИЕ_УЛУЧШЕНИЯ', 'ЗАВЕРШЕНИЕ_ХОДА')
   ...
   begin
      ...
      update "ИГРЫ"
         set "КОД_СТАТУСА_ИГРЫ" = 'ЗАВЕРШЕНА', "ДАТА_ЗАВЕРШЕНИЯ" = sysdate, "ID_ПОБЕДИТЕЛЯ" = p_winner_id,
             "ID_ТЕКУЩЕГО_УЧАСТНИКА" = null, "КОД_СОСТОЯНИЯ_ХОДА" = null, "ВРЕМЯ_НАЧАЛА_ХОДА" = null
       where "ID_ИГРЫ" = p_game_id;
   end;

   procedure finish_or_continue(p_game_id number) is
      v_count number;
      winner number;
   begin
      ...
      if v_count = 1 then
         finish_game(p_game_id, winner);
      elsif v_count = 0 then
         finish_game(p_game_id, null);
      end if;
   end;
   ...
   begin
      ...
      on x."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      update "УЧАСТНИКИ"
         set "КОД_СТАТУСА_УЧАСТНИКА" = 'БАНКРОТ'
       where "ID_УЧАСТНИКА" = p_participant_id;
      return_properties_to_bank(p_participant_id);
      add_action(v_game_id, p_participant_id, null, 'БАНКРОТСТВО', v_balance);
   ...
   procedure leave_active_game(p_participant_id number) is
      v_game_id number;
      v_game_status varchar2(40);
      v_count number;
   begin
   ...
   procedure disconnect_player(p_participant_id number) is
      v_game_id number;
      v_current_player_id number;
      v_game_status varchar2(40);
      auction_id number;
   begin
      ...
      return_properties_to_bank(p_participant_id);
      add_action(v_game_id, p_participant_id, null, 'ВЫХОД_УЧАСТНИКА', null);
      add_action(v_game_id, p_participant_id, null, 'БАНКРОТСТВО', null);
      finish_or_continue(v_game_id);
      select "КОД_СТАТУСА_ИГРЫ" into v_game_status
        from "ИГРЫ"
       where "ID_ИГРЫ" = v_game_id;
   ...
   procedure send_message(p_participant_id number, p_text varchar2) is
      v_game_id number;
      v_player_status varchar2(40);
      gs varchar2(40);
   begin
      ...
      select u."КОД_СТАТУСА_УЧАСТНИКА", games."КОД_СТАТУСА_ИГРЫ"
        into v_player_status, gs
        from "УЧАСТНИКИ" u
        join "ИГРЫ" games
      on games."ID_ИГРЫ" = u."ID_ИГРЫ"
       where u."ID_УЧАСТНИКА" = p_participant_id;
      if v_player_status not in ('В_ЛОББИ', 'АКТИВЕН', 'БАНКРОТ')
      or gs = 'ЗАБРОШЕНА' then
         raise_application_error(-20111, 'Чат недоступен');
      end if;
      insert into "ЧАТ" ("ID_УЧАСТНИКА", "ТЕКСТ", "ДАТА_ВРЕМЯ") values
         (p_participant_id, trim(p_text), sysdate);
   end;

   function get_chat(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
      v_game_id number;
      v_player_status varchar2(40);
   begin
      select "ID_ИГРЫ", "КОД_СТАТУСА_УЧАСТНИКА"
        into v_game_id, v_player_status
        from "УЧАСТНИКИ"
       where "ID_УЧАСТНИКА" = p_participant_id;
      if v_player_status not in ('В_ЛОББИ', 'АКТИВЕН', 'БАНКРОТ') then
         raise_application_error(-20112, 'Чат недоступен');
      end if;
      open v_result for
         select c."ID_СООБЩЕНИЯ", p."ЛОГИН", c."ТЕКСТ", c."ДАТА_ВРЕМЯ"
         from "ЧАТ" c
         join "УЧАСТНИКИ" u
         ...
         join "ПОЛЬЗОВАТЕЛИ" p
            on p."ID_ПОЛЬЗОВАТЕЛЯ" = u."ID_ПОЛЬЗОВАТЕЛЯ"
         where u."ID_ИГРЫ" = v_game_id
         order by c."ДАТА_ВРЕМЯ", c."ID_СООБЩЕНИЯ";
      return v_result;
   end;

   function get_action_log(p_participant_id number) return sys_refcursor is
      v_result sys_refcursor;
      v_game_id number;
   begin
      v_game_id := participant_game(p_participant_id);
      open v_result for
         ...
         from (
            select j."ID_ДЕЙСТВИЯ", j."ДАТА_ВРЕМЯ", j."КОД_ДЕЙСТВИЯ",
               t."НАИМЕНОВАНИЕ" "ДЕЙСТВИЕ", p."ЛОГИН", c."НАЗВАНИЕ" "КЛЕТКА", j."СУММА",
               case when j."КОД_ДЕЙСТВИЯ" = 'ОПЛАТА_АРЕНДЫ' then j."ТЕКСТ_СОБЫТИЯ" end "ПОЛУЧАТЕЛЬ"
            from "ЖУРНАЛ_ДЕЙСТВИЙ" j
            left join "УЧАСТНИКИ" u
   ...
   procedure get_game_snapshot(
      ...
      p_state out sys_refcursor, p_players out sys_refcursor,
      p_cells out sys_refcursor, p_ownerships out sys_refcursor,
      p_actions out sys_refcursor, p_chat out sys_refcursor
   ) is
      v_game_id number;
   begin
      v_game_id := participant_game(p_participant_id);
      ...
      open p_cells for
         select c."ID_КЛЕТКИ", c."ПОЗИЦИЯ", c."НАЗВАНИЕ", c."ТИП", c."ЦВЕТОВАЯ_ГРУППА",
            c."ЦЕНА_ПОКУПКИ", c."ЦЕНА_ПОКУПКИ" "БАЗОВАЯ_РЕНТА",
            ...
            case when c."ТИП" = 'Старт' then c_start_bonus end "БОНУС_СТАРТА"
         from "КЛЕТКИ" c
         order by c."ПОЗИЦИЯ";

      open p_ownerships for
         select v."ID_КЛЕТКИ", v."ID_ВЛАДЕНИЯ", v."ID_ВЛАДЕЛЬЦА", p."ЛОГИН" "ВЛАДЕЛЕЦ",
            v."КОЛВО_ДОМОВ", v."ЗАЛОЖЕНА",
            case
               when c."ТИП" = 'Улица'
      ...
      open p_actions for
         select j."ID_ДЕЙСТВИЯ", j."ДАТА_ВРЕМЯ", j."КОД_ДЕЙСТВИЯ", t."НАИМЕНОВАНИЕ" "ДЕЙСТВИЕ",
            p."ЛОГИН", c."НАЗВАНИЕ" "КЛЕТКА", j."СУММА", j."ТЕКСТ_СОБЫТИЯ",
            case when j."КОД_ДЕЙСТВИЯ" = 'ОПЛАТА_АРЕНДЫ' then j."ТЕКСТ_СОБЫТИЯ" end "ПОЛУЧАТЕЛЬ"
         from "ЖУРНАЛ_ДЕЙСТВИЙ" j
         left join "УЧАСТНИКИ" u
            on u."ID_УЧАСТНИКА" = j."ID_УЧАСТНИКА"
         ...
         join "ТИПЫ_ДЕЙСТВИЙ" t
            on t."КОД_ДЕЙСТВИЯ" = j."КОД_ДЕЙСТВИЯ"
         where j."ID_ИГРЫ" = v_game_id
            and j."ID_ДЕЙСТВИЯ" > nvl(p_last_action_id, 0)
         order by j."ID_ДЕЙСТВИЯ";

      open p_chat for
         select ch."ID_СООБЩЕНИЯ", p."ЛОГИН", ch."ТЕКСТ", ch."ДАТА_ВРЕМЯ"
         from "ЧАТ" ch
         join "УЧАСТНИКИ" u
         ...
         where u."ID_ИГРЫ" = v_game_id
            ...
            and ch."ID_СООБЩЕНИЯ" > nvl(p_last_message_id, 0)
         order by ch."ID_СООБЩЕНИЯ";
   end;

   function get_player_stats(p_user_id number) return sys_refcursor is
      v_result sys_refcursor;
   begin
      open v_result for
         with played as (
            select u."ID_УЧАСТНИКА", u."ID_ИГРЫ", games."ID_ПОБЕДИТЕЛЯ"
            from "УЧАСТНИКИ" u
            join "ИГРЫ" games
               on games."ID_ИГРЫ" = u."ID_ИГРЫ"
            where u."ID_ПОЛЬЗОВАТЕЛЯ" = p_user_id
            ...
            select c."НАЗВАНИЕ", count(*) visit_count, row_number()
               over(order by count(*) desc, c."ПОЗИЦИЯ") rn
            from "ЖУРНАЛ_ДЕЙСТВИЙ" j
            join "УЧАСТНИКИ" u
               on u."ID_УЧАСТНИКА" = j."ID_УЧАСТНИКА"
            join "КЛЕТКИ" c
            ...
            where u."ID_ПОЛЬЗОВАТЕЛЯ" = p_user_id
               ...
               and games."КОД_СТАТУСА_ИГРЫ" = 'ЗАВЕРШЕНА'
            group by c."НАЗВАНИЕ", c."ПОЗИЦИЯ"
         )
         select p."ЛОГИН", count(pl."ID_ИГРЫ") "КОЛИЧЕСТВО_ИГР", nvl(
               sum(case when pl."ID_ПОБЕДИТЕЛЯ" = pl."ID_УЧАСТНИКА" then 1 else 0 end), 0
         ) "ПОБЕДЫ",
            case
               when count(pl."ID_ИГРЫ") = 0 then 0
               else
               round(
                  100 * sum(case when pl."ID_ПОБЕДИТЕЛЯ" = pl."ID_УЧАСТНИКА" then 1 else 0 end) / count(pl."ID_ИГРЫ"), 2
               )
            end "ПРОЦЕНТ_ПОБЕД", (
            select "НАЗВАНИЕ"
            from fav
   ...
   begin
      open v_result for
         select p."ID_ПОЛЬЗОВАТЕЛЯ", p."ЛОГИН", count(games."ID_ИГРЫ") "ИГРЫ", nvl(
               sum(case when games."ID_ПОБЕДИТЕЛЯ" = u."ID_УЧАСТНИКА" then 1 else 0 end), 0
         ) "ПОБЕДЫ",
            case
               when count(games."ID_ИГРЫ") = 0 then 0
               else
               round(
                  100 * sum(case when games."ID_ПОБЕДИТЕЛЯ" = u."ID_УЧАСТНИКА" then 1 else 0 end) / count(games."ID_ИГРЫ"), 2
               )
            end "ПРОЦЕНТ"
         from "ПОЛЬЗОВАТЕЛИ" p
         left join "УЧАСТНИКИ" u
         ...
         left join "ИГРЫ" games
            on games."ID_ИГРЫ" = u."ID_ИГРЫ"
            and games."КОД_СТАТУСА_ИГРЫ" = 'ЗАВЕРШЕНА'
         group by p."ID_ПОЛЬЗОВАТЕЛЯ", p."ЛОГИН"
         order by "ПРОЦЕНТ" desc, "ПОБЕДЫ" desc, "ИГРЫ" desc, p."ЛОГИН";
      return v_result;
   end;

   function get_game_history(p_user_id number) return sys_refcursor is
      v_result sys_refcursor;
   begin
      open v_result for
         select games."ID_ИГРЫ", games."НАЗВАНИЕ", games."ДАТА_СТАРТА", games."ДАТА_ЗАВЕРШЕНИЯ",
            u."БАЛАНС" "ИТОГОВЫЙ_БАЛАНС", u."КОД_СТАТУСА_УЧАСТНИКА", wp."ЛОГИН" "ПОБЕДИТЕЛЬ",
            case when games."ID_ПОБЕДИТЕЛЯ" = u."ID_УЧАСТНИКА" then 'Победа' else 'Поражение' end "РЕЗУЛЬТАТ"
         from "УЧАСТНИКИ" u
         join "ИГРЫ" games
            on games."ID_ИГРЫ" = u."ID_ИГРЫ"
