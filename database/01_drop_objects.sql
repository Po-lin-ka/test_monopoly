BEGIN
  ...
  FOR t IN (SELECT table_name FROM user_tables WHERE table_name IN (
    'ЧАТ','ЖУРНАЛ_ДЕЙСТВИЙ','СТАВКИ','АУКЦИОНЫ','ВЛАДЕНИЯ','КАРТЫ_ШАНСА','УЧАСТНИКИ','ИГРЫ','КЛЕТКИ','ПОЛЬЗОВАТЕЛИ','СОСТОЯНИЯ_ХОДА','ТИПЫ_ДЕЙСТВИЙ','СТАТУСЫ_АУКЦИОНОВ','СТАТУСЫ_УЧАСТНИКОВ','СТАТУСЫ_ИГР')) LOOP
